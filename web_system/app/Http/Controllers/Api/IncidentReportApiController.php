...
class IncidentReportApiController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if ($transmissionType === 'online' && $request->hasFile('evidence') && $evidenceType !== 'none') {
            $photoEvidenceUpload = $evidenceType === 'photo' ? $request->file('evidence') : null;
        ...
        if ($user->isCivilian() && $photoEvidenceUpload !== null) {
        ...
        if ($user->isCivilian() && $shouldAnalyzeImage && $photoEvidenceUpload !== null) {
            try {
                $screening = app(AiSeverityClient::class)->analyzeUploadedEvidence($photoEvidenceUpload);

                if (! ($screening['accepted'] ?? true)) {
                    throw ValidationException::withMessages([
                        'evidence' => $screening['rejection_message'] ?? 'Upload a real accident or emergency scene photo.',
                    ]);
                }

                if (is_array($screening['analysis'] ?? null)) {
        ...
        $report = IncidentReport::create([
            ...
            'ai_status' => $shouldAnalyzeImage ? 'pending' : $analysis['status'],
    ...
    private function reportPayload(IncidentReport $report): array
    {
        return [
            ...
            'hardware_source' => $report->hardware_source,
            'lora_sender_id' => $report->lora_sender_id,
            'lora_sequence' => $report->lora_sequence,
            'lora_gateway_id' => $report->lora_gateway_id,
            'lora_gateway_rssi' => $report->lora_gateway_rssi,
            'lora_gateway_snr' => $report->lora_gateway_snr,
            'lora_receiver_rssi' => $report->lora_receiver_rssi,
            'lora_receiver_snr' => $report->lora_receiver_snr,
            'lora_satellites' => $report->lora_satellites,

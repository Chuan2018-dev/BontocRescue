                ...
                @forelse ($reports as $report)
                    ...
                    @if ($isCivilian)
                        ...
                        <article class="civilian-history-card">
                            ...
                            <p>{{ \Illuminate\Support\Str::limit($report->description, 185) }}</p>

                            <div class="civilian-report-stats">
                                <article class="civilian-stat-card">
                                    <span>Track this report</span>
                                    <strong>{{ $civilianStatusSummary($report->status) }}</strong>
                                </article>
                                <article class="civilian-stat-card">
                                    <span>Estimated response time</span>
                                    <strong>{{ $estimatedResponse['label'] }} · {{ $estimatedResponse['time'] }}</strong>
                                </article>
                                <article class="civilian-stat-card">
                                    <span>Attachments</span>
                                    <strong>{{ $civilianAttachmentSummary($report) }}</strong>
                                </article>
                            </div>

                            <div class="civilian-progress-block">
                                <div class="tag-row" style="justify-content:space-between;align-items:center;">
                                    <span class="incident-code">Status timeline</span>
                                    <span class="tag blue">{{ $civilianProgressPercent($report->status) }}% complete</span>
                                </div>
                                <div class="civilian-progress-track">
                                    <div class="civilian-progress-fill {{ $report->status === 'resolved' ? 'is-complete' : ($report->status === 'rejected' ? 'is-rejected' : '') }}" style="width:{{ $civilianProgressPercent($report->status) }}%;"></div>
                                </div>
                                <span class="incident-code">Progress tracker</span>
                            </div>
                    ...
                    @else
                        <article class="incident-card responder-feed-card">
                            ...
                            <div class="incident-content">
                                ...
                                <div class="dual-grid responder-feed-actions-grid">

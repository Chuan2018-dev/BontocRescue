...
@section('hero')
    <section class="hero-card" style="{{ $isCivilian ? 'background:linear-gradient(135deg,rgba(255,255,255,.96),rgba(232,244,255,.95));border-color:rgba(32,104,174,.16);' : '' }}">
        <div class="hero-grid">
            ...
            @if ($isCivilian)
                <div class="stack">
                    <div class="hero-metrics">
                        <article class="metric-card"><span>Total reports</span><strong>{{ $reportStats['total'] }}</strong><p>Reports submitted from your civilian account.</p></article>
                        <article class="metric-card"><span>Active reports</span><strong>{{ $reportStats['active'] }}</strong><p>Reports still under active responder handling.</p></article>
            ...
            @else
                <div class="hero-metrics">
                    <article class="metric-card"><span>Total reports</span><strong>{{ $reportStats['total'] }}</strong><p>Reports linked to your account activity.</p></article>
                    <article class="metric-card"><span>Active reports</span><strong>{{ $reportStats['active'] }}</strong><p>Reports still open or under review.</p></article>
                    <article class="metric-card"><span>Completed</span><strong>{{ $reportStats['completed'] }}</strong><p>Reports already marked as completed.</p></article>
                    <article class="metric-card"><span>Callsign</span><strong>{{ $callsign }}</strong><p>{{ $stationLabel }}</p></article>
                </div>
            @endif
...
@section('content')
        ...
        <section class="civilian-shell">
            ...
            <section class="civilian-home-grid">
                <article class="panel">
                    ...
                    <div class="stack">
                        <div class="civilian-helper-card" style="justify-items:center;text-align:center;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(232,244,255,.96));border-color:rgba(32,104,174,.16);">
                            @if ($profilePhotoUrl)
                            ...
                            @endif
                            <span class="tag blue">{{ $roleLabel }}</span>
                            <strong style="font-size:1.25rem;">{{ $user->name }}</strong>
                            <p>{{ $user->email }}</p>
                            <div class="civilian-pill-list">
                                <span class="civilian-pill">{{ $callsign }}</span>
                                <span class="civilian-pill">{{ $stationLabel }}</span>
                            </div>
                        </div>
                ...
                <div class="stack">
                    <article class="civilian-helper-card">
                        ...
                        <div class="civilian-report-stats">
                            ...
                            <article class="civilian-stat-card">
                                <span>Phone</span>
                                <strong>{{ $phone !== '' ? $phone : 'Not yet configured' }}</strong>
                            </article>
                            <article class="civilian-stat-card">
                                <span>Emergency contact</span>
                                <strong>{{ $emergencyContactName !== '' ? $emergencyContactName : 'Not yet configured' }}</strong>
                            </article>
                            <article class="civilian-stat-card">
                                <span>Emergency phone</span>
                                <strong>{{ $emergencyContactPhone !== '' ? $emergencyContactPhone : 'Not yet configured' }}</strong>
                            </article>
                        </div>
                    </article>
                            ...
                            <div class="civilian-history-top">
                                <div class="civilian-history-meta">
                                    ...
                                    <div>
                                        ...
                                        <p><strong>{{ $report->location_text }}</strong></p>
                                    </div>
                                </div>
                                <a href="{{ route('reports.show', $report) }}" class="btn btn-secondary">Open</a>
                            </div>
                        </article>
                    @empty
                        <div class="civilian-empty-state">
                            <strong>No recent report activity yet.</strong>
                            <p>No report history is available for this account yet.</p>
                        </div>
                    @endforelse
                </div>
            </section>
    ...
    @else
        <section class="dual-grid profile-shell-grid">
            <article class="panel">
                <div class="panel-head">
                    <div class="panel-heading">
                        <p class="panel-kicker">Profile Snapshot</p>
                ...
                <div class="stack">
                    <div class="detail-card" style="justify-items:center;text-align:center;background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(232,244,255,.96));border-color:rgba(32,104,174,.16);">
                        ...
                        @else
                            <div style="width:156px;height:156px;border-radius:28px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#153c5e,#1f6ca8);color:#fff;font-size:2.6rem;font-weight:900;letter-spacing:.08em;">{{ $initials }}</div>
                        @endif
                        <span class="tag {{ $user?->is_admin ? 'red' : 'green' }}">{{ $roleLabel }}</span>
                        <strong style="font-size:1.25rem;">{{ $user->name }}</strong>
                        <p>{{ $user->email }}</p>
                    </div>
                    ...
                    <div class="detail-card">
                        ...
                        <p><strong>Assigned station:</strong> {{ $stationLabel }}</p>
                        <p><strong>Callsign:</strong> {{ $callsign }}</p>
                    </div>

                    <div class="detail-card">
                        <strong>History of Report</strong>
                        <p>Review report activity linked to your responder account.</p>
                        <div class="action-row">
                            <a href="{{ route('reports.index') }}" class="action-btn secondary">Open Report History</a>
                            <a href="{{ route('reports.create') }}" class="action-btn secondary">Create Report</a>
                        </div>
                    </div>
                </div>
            </article>
